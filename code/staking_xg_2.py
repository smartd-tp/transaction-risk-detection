...
xgb_stack.fit(np.c_[stacking_input[:,2:],stacking_lr_input[:,1:],stacking_rf_input[:,1:]], train_sample['Label'],eval_metric='auc')
        
#Predict training set:
dtrain_predictions = xgb_stack.predict(np.c_[stacking_input[:,2:],stacking_lr_input[:,1:],stacking_rf_input[:,1:]]) ### 训练集
pred_y = xgb_stack.predict(np.c_[test_set[:,2:],test_lr_set[:,1:],test_rf_set[:,1:]]) ### 测试集
...
out=pd.DataFrame({'ID':pred['ID'],'Label':lr_pred_train})

out.to_csv("/Users/cp/Documents/job/smartdec/AL_COMPETITION/workfile/xg_stack_4.csv",index=False,sep=',')
